    ...
    <div class="main-content">
        <section class="section">
            ...
            <div class="section-body">
                <a href="{{ route('admin.faq.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i>
                    {{ __('admin.Add New') }}</a>
                <div class="row mt-4">
                    <div class="col">
                        <div class="card">
                                    ...
                                    <table class="table table-striped" id="dataTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="10%">{{ __('admin.Translations') }}</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @forelse($languages as $key => $language)
                                                            <i
                                                                class="fa {{ $faq->translation($language->code)?->first()?->answer ? 'fa-check' : 'fa-edit' }}"></i>
                                                            <a
                                                                href="{{ route('admin.faq.translation', ['id' => $faq->id, 'code' => $language->code]) }}">{{ strtoupper($language->code) }}</a>
                                                            @if (!$loop->last)
                                                                ||
                                                            @endif
                                                        @empty
                                                            <a
                                                                href="{{ route('admin.faq.translation', ['id' => $faq->id, 'code' => $language->code]) }}">{{ strtoupper(config('app.locale')) }}</a>
                                                        @endforelse
                                                    </td>
                                                    <td>
                                                        @if ($faq->status == 1)
