...
class PaymentMethodController extends Controller
{
    ...
    public function index(){
        ...
        $mercadopago = MercadopagoPayment::first();

        $countires = CurrencyCountry::orderBy('name','asc')->get();
        $currencies = Currency::orderBy('name','asc')->get();
        $setting = Setting::first();

        return view('admin.payment_method', compact('paypal','stripe','razorpay','bank','paystackAndMollie','flutterwave','instamojo','countires','currencies','setting','paymongo','mercadopago'));
    ...
    public function updateMercadoPago(Request $request){
        $rules = [
            ...
            'access_token' => $request->status ? 'required' : '',
        ...
        $mercadopago->currency_rate = $request->currency_rate;
        $mercadopago->country_code = $request->country_name;
        $mercadopago->currency_code = $request->currency_name;
        $mercadopago->status = $request->status ? 1 : 0;
        $mercadopago->save();
